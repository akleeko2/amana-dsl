...
fn main() {
    ...
    match command {
        ...
        CliCommand::Generate {
            ...
            let filename = format!("{}.amana", name);
            let path = Path::new(&filename);
            if path.exists() {
                eprintln!("{} Error: File '{}' already exists.", color_red("[Amana]"), filename);
                process::exit(1);
            }

            let boilerplate = match (element_type.as_str(), pattern.as_deref()) {
                ("page", Some("PricingSection")) => {
                    format!(
                        r#"# Pricing page generated by Amana CLI
theme:
    preset: stripe
    radius: soft
    density: default
...
view {}:
    render:
        Container:
            PricingSection(title: "الباقات والأسعار", billing: "monthly"):
                plan(name: "المجاني", price: "0 ر.س", cta: "ابدأ مجاناً"):
                    feature: "ميزات أساسية"
                    feature: "دعم عبر البريد"
                plan(name: "الاحترافي", price: "99 ر.س", highlight: true, cta: "اشترك الآن"):
                    feature: "كل ميزات الباقة المجانية"
                    feature: "دعم على مدار الساعة"
                    feature: "تقارير متقدمة"
"#,
                        name, name, name
                    )
                }
                ("page", Some("DashboardShell")) => {
                    format!(
                        r#"# Dashboard page generated by Amana CLI
theme:
    preset: linear
    radius: soft
    density: compact
...
view {}:
    render:
        DashboardShell(brand: "لوحة تحكم أمانة", user: "المطور"):
            sidebar:
                a(href: "/dashboard"): "الرئيسية"
                a(href: "/analytics"): "التحليلات"
                a(href: "/settings"): "الإعدادات"
            main:
                Container:
                    Section(title: "أهلاً بك في لوحة التحكم"):
                        p: "هنا يمكنك البدء في تخصيص تطبيقك بالكامل."
"#,
                        name, name, name
                    )
                }
                ("page", Some("AuthPage")) => {
                    format!(
                        r#"# Auth page generated by Amana CLI
theme:
    preset: luxury
    radius: soft
    density: default
...
view {}:
    render:
        AuthPage(type: "login", title: "مرحباً بك مجدداً", logo: "/logo.svg")
"#,
                        name, name, name
                    )
                }
                ("page", Some("SettingsPage")) => {
                    format!(
                        r#"# Settings page generated by Amana CLI
theme:
    preset: stripe
    radius: soft
    density: default
...
view {}:
    render:
        Container:
            SettingsPage(title: "إعدادات الحساب", description: "إدارة إعدادات الأمان والتفضيلات الشخصية"):
                section(title: "الملف الشخصي", description: "تحديث معلوماتك الشخصية"):
                    FormField(label: "الاسم الكامل", name: "name")
                    FormField(label: "البريد الإلكتروني", name: "email")
                section(title: "الأمان", description: "تغيير كلمة المرور وتفعيل ميزات الأمان"):
                    FormField(label: "كلمة المرور الحالية", name: "current_password", type: "password")
                    FormField(label: "كلمة المرور الجديدة", name: "new_password", type: "password")
"#,
                        name, name, name
                    )
                }
...
view {}:
    render:
        Container:
            Section(title: "صفحة {}"):
                p: "هذه صفحة جديدة فارغة تم إنشاؤها عبر CLI."
"#,
                        name, name, name, name, name
                    )
                }
                ("component", _) => {
                    let model_content = if let Some(m) = model {
                        format!(" (ربط مع النموذج: {})", m)
                    } else {
                        String::new()
                    };
                    format!(
                        r#"# Component {} generated by Amana CLI {}
component {}:
    Card(title: "مكون {}"):
        p: "هذا مكون مخصص تم توليده تلقائياً."
"#,
                        name, model_content, name, name
                    )
                }
                _ => "".to_string(),
            };

            match fs::write(path, boilerplate) {
                Ok(_) => {
                    println!(
...
fn parse_cli_args(args: &[String]) -> Result<CliCommand, String> {
    ...
    match args[1].as_str() {
        ...
        "generate" | "g" => parse_generate_args(args),
...
fn parse_generate_args(args: &[String]) -> Result<CliCommand, String> {
    if args.len() < 4 {
        return Err("Usage: amana generate <page|component> <name> [--pattern <pattern>] [--model <model>]".to_string());
    }
    let element_type = args[2].clone();
    if element_type != "page" && element_type != "component" {
        return Err(format!("Invalid element type '{}'. Must be 'page' or 'component'.", element_type));
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--pattern" => {
                if i + 1 >= args.len() {
                    return Err("Missing value for flag --pattern".to_string());
                }
            ...
            "--model" => {
                if i + 1 >= args.len() {
                    return Err("Missing value for flag --model".to_string());
                }
                model = Some(args[i + 1].clone());
                i += 2;
            }
            _ => return Err(format!("Unknown generate flag '{}'.", args[i])),
        }
    }
...
fn print_usage() {
    ...
    println!("  amana generate <page|component> <name> [--pattern <pattern>] [--model <model>]");
